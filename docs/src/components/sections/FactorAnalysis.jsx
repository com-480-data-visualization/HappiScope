...
const FactorAnalysis = () => {
  ...
  const factorDescriptions = {
    "GDP per Capita":
      "Measures the economic strength of a country divided by its population. Higher GDP generally correlates with better living standards.",
    "Social Support":
      "Indicates whether people have someone to count on in times of trouble. Strong social networks significantly contribute to wellbeing.",
    "Life Expectancy":
      "Reflects healthy life expectancy and the overall quality of healthcare systems in a country.",
    Freedom:
      "Measures the degree to which people feel they have freedom to make life choices. Personal autonomy is central to happiness.",
    Generosity:
      "Reflects the prevalence of charitable donations and helping behaviors in society. Giving has benefits both for recipients and givers.",
    Corruption:
      "Measures perception of corruption in government and business. Higher values indicate lower corruption levels (more trust), which is associated with higher life satisfaction.",
  };
  ...
  useEffect(() => {
    try {
      ...
      const regionalData = continents
        .map((continent) => {
          const continentData = continentGroups[continent];
          // Average across all years
          const factorValues = continentData
            .map((d) => d[factorKey])
      ...
      const timeData = years.map((year) => {
        ...
        allContinents.forEach((continent) => {
          const continentYearData = yearData.filter(
            (d) => d.continent === continent
          );
          const continentAvg =
            continentYearData.length > 0
              ? continentYearData
                  .map((d) => d[factorKey])
                  .filter((v) => v !== undefined && !isNaN(v))
                  .reduce((sum, val) => sum + val, 0) / continentYearData.length
              : 0;
          continentData[continent.toLowerCase().replace(" ", "_")] =
            continentAvg;
        });
      ...
      const impactData = incomeGroups.map((group) => {
        const groupData = happinessData.filter(
          (d) => d.development_category === group
        );
        const factorValues = groupData
          .map((d) => d[factorKey])
          .filter((v) => v !== undefined && !isNaN(v));
        const avg =
          factorValues.length > 0
  ...
  const LoadingPlaceholder = () => (
    <div className="flex flex-col items-center justify-center h-full">
      ...
      <p className="text-gray-400 text-xs mt-1">
        Analyzing {selectedFactor} patterns
      </p>
    </div>
  );
  ...
  const getScatterPlotData = useCallback(() => {
    ...
    const validData = happinessData.filter(
      (d) =>
        ...
        !isNaN(d.score) &&
        d.continent &&
        d.continent !== null && // Exclude data points with null continent
  ...
  const handleTooltip = useCallback(
        ...
        setTooltip({
          content: (
            <>
              ...
              <div>
                {node.data.continent} ({node.data.year})
              </div>
    ...
    [selectedFactor]
  ); // Add dependencies that the tooltip content uses
  ...
  // Custom tooltip for time trend that shows all continents for a given year
  const TimeTooltip = useCallback(
    ({ slice }) => {
      if (!slice || !slice.points || slice.points.length === 0) return null;

      // Get the year from the first point (all points in the slice have the same x-value)
      ...
      <section className="section-container py-12">
        ...
        >
          <h1 className="section-title text-center">
            Happiness Factor Analysis
          </h1>
          <p className="text-gray-600 text-center mb-8 max-w-3xl mx-auto">
            Explore the relationship between happiness scores and various
            contributing factors across different regions and time periods.
            Compare how each factor impacts overall happiness.
          </p>
        </motion.div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          >
            ...
            <p className="text-gray-600">
              ...
              {selectedFactor.toLowerCase()} and overall happiness scores across
              countries. Each bubble represents a country, with size indicating
              population and colors showing different continents. The
              correlation coefficient is {factorCorrelation}.
            </p>
          </motion.div>
          ...
          >
            ...
            <p className="text-gray-600">
              This bar chart compares {selectedFactor.toLowerCase()} across
              different continents, highlighting regional variations.{" "}
              {selectedFactor === "Corruption"
                ? "Higher values indicate lower corruption (better outcomes)"
          ...
          >
            ...
            >
              {isLoading ? (
                ...
                <ResponsiveLine
                  ...
                  enableArea={false} // Removed shaded areas under curves
            ...
            <p className="text-gray-600">
              This line chart tracks {selectedFactor.toLowerCase()} trends over
              the past decade across all continents.
                ...
                (() => {
                  ...
                  continentKeys.forEach((key) => {
                    if (firstYear[key] && lastYear[key]) {
                      const change =
                        ((lastYear[key] - firstYear[key]) / firstYear[key]) *
                        100;
                  ...
                  if (maxIncrease.key && Math.abs(maxIncrease.change) > 5) {
                    return ` ${formattedRegion(
                      maxIncrease.key
                    )} shows the most improvement (+${maxIncrease.change.toFixed(
                      1
                    )}%)${
                      maxDecrease.key
                        ? `, while ${formattedRegion(
                            maxDecrease.key
                          )} shows the greatest decline (${maxDecrease.change.toFixed(
                            1
                          )}%)`
                        : ""
                    }.`;
          ...
          >
            ...
            <p className="text-gray-600">
              This visualization shows how the contribution of{" "}
              {selectedFactor.toLowerCase()} to happiness varies across
              different development categories.
              {factorImpactData.length > 0 &&
                (factorImpactData[0].proportion >
                factorImpactData[factorImpactData.length - 1].proportion
                  ? ` Note how this factor is more influential in ${
                      factorImpactData[0].group
                    } development countries compared to ${
                      factorImpactData[factorImpactData.length - 1].group
        ...
        >
          <h2 className="text-2xl font-semibold mb-6 text-center text-gray-800">
            Key Findings About {selectedFactor}
          </h2>
                  ...
                  <div
                    className={`w-10 h-10 rounded-full flex items-center justify-center mr-3 ${
                      selectedFactor === "GDP per Capita"
                        ...
                        : selectedFactor === "Freedom"
                        ? "bg-lightGreen-100 text-green-600"
                        : selectedFactor === "Generosity"
                ...
                <div className="ml-13 pl-0">
                  ...
                  {selectedFactor === "GDP per Capita" && (
                    <p className="text-gray-600 leading-relaxed">
                      GDP per capita shows a strong correlation with happiness
                      but exhibits
                      <span className="font-medium"> diminishing returns</span>.
                      Data reveals a logarithmic relationship where each
                      additional $10,000 yields progressively smaller happiness
                      gains beyond $40,000 per capita.
                    </p>
                  )}
                  {selectedFactor === "Social Support" && (
                    <p className="text-gray-600 leading-relaxed">
                      Social support is among the most consistent predictors of
                      happiness globally. Quantitative analysis shows that a
                      <span className="font-medium">
                        {" "}
                        0.1-point increase in social support
                      </span>{" "}
                      correlates with a 0.3-0.5 point increase in happiness
                      scores, regardless of economic development.
                    </p>
                  )}
                  {selectedFactor === "Life Expectancy" && (
                    <p className="text-gray-600 leading-relaxed">
                      Health demonstrates a powerful correlation with happiness.
                      Data analysis reveals that
                      <span className="font-medium">
                        {" "}
                        each additional year of healthy life expectancy
                      </span>{" "}
                      correlates with approximately a 0.029-point increase in
                      happiness scores across nations.
                    </p>
                  )}
                  {selectedFactor === "Freedom" && (
                    <p className="text-gray-600 leading-relaxed">
                      Freedom to make life choices shows a significant
                      correlation with happiness. Statistical analysis reveals
                      that freedom scores explain approximately
                      <span className="font-medium">
                        {" "}
                        {(
                          parseFloat(factorCorrelation) *
                          parseFloat(factorCorrelation) *
                          100
                        ).toFixed(1)}
                        % of variation
                      </span>{" "}
                      in happiness levels globally, with particularly strong
                      effects in democratic societies.
                    </p>
                  )}
                  {selectedFactor === "Generosity" && (
                    <p className="text-gray-600 leading-relaxed">
                      Generosity exhibits a more modest correlation with
                      national happiness. Data analysis shows that while
                      generosity's direct effect appears limited, it has
                      <span className="font-medium">
                        {" "}
                        indirect benefits
                      </span>{" "}
                      through strengthening social capital and community
                      resilience.
                    </p>
                  )}
                  {selectedFactor === "Corruption" && (
                    <p className="text-gray-600 leading-relaxed">
                      Perception of corruption shows a significant correlation
                      with happiness. Regression analysis indicates that each
                      <span className="font-medium">
                        {" "}
                        0.1-point improvement
                      </span>{" "}
                      in corruption scores (indicating lower corruption)
                      correlates with approximately a 0.2-0.3 point increase in
                      happiness.
                    </p>
                  )}
                </div>
              ...
              <div className="p-6">
                ...
                {factorRegionalData.length > 0 &&
                  (() => {
                    ...
                    const highestDiff = (
                      ((highest.value - avg) / avg) *
                      100
                    ).toFixed(1);
                    const lowestDiff = (
                      ((lowest.value - avg) / avg) *
                      100
                    ).toFixed(1);
                      ...
                      <div className="ml-13 pl-0">
                        <div className="flex flex-wrap gap-3 mb-4">
                          ...
                          <div className="bg-gray-50 rounded-lg px-3 py-1 text-gray-700 text-sm font-semibold">
                            Gap:{" "}
                            {Math.abs(highest.value - lowest.value).toFixed(2)}{" "}
                            points
                          </div>
                        </div>

                        <p className="text-gray-600 leading-relaxed">
                          Regional data reveals significant disparities in{" "}
                          {selectedFactor.toLowerCase()} levels.{" "}
                          {highest.continent} leads with
                          <span className="font-medium">
                            {" "}
                            {highest.value.toFixed(2)} ({highestDiff}% above
                            average)
                          </span>
                          , while {lowest.continent} records
                          <span className="font-medium">
                            {" "}
                            {lowest.value.toFixed(2)} (
                            {Math.abs(parseFloat(lowestDiff))}%{" "}
                            {parseFloat(lowestDiff) > 0 ? "above" : "below"}{" "}
                            average)
                          </span>
                          ...
                          {selectedFactor === "GDP per Capita"
                            ...
                            : selectedFactor === "Freedom"
                            ? " political systems and civic freedoms."
                            : selectedFactor === "Generosity"
                            ? " cultural values and charitable behaviors."
              ...
              <div className="p-6">
                ...
                {factorTimeData.length > 1 &&
                  (() => {
                    ...
                    const lastYear = factorTimeData[factorTimeData.length - 1];
                    const globalChange =
                      ((lastYear.global - firstYear.global) /
                        firstYear.global) *
                      100;
                    ...
                    return (
                      <div className="ml-13 pl-0">
                        <div className="flex flex-wrap gap-2 mb-4">
                          ...
                          {maxIncrease.key &&
                            Math.abs(maxIncrease.change) > 5 && (
                              <div className="bg-green-50 rounded-lg px-3 py-1 text-green-700 text-sm font-semibold">
                                ↑ {formattedRegion(maxIncrease.key)}: +
                                {maxIncrease.change.toFixed(1)}%
                              </div>
                            )}
                          ...
                          {maxDecrease.key &&
                            Math.abs(maxDecrease.change) > 5 && (
                              <div className="bg-red-50 rounded-lg px-3 py-1 text-red-700 text-sm font-semibold">
                                ↓ {formattedRegion(maxDecrease.key)}:{" "}
                                {maxDecrease.change.toFixed(1)}%
                              </div>
                            )}
                        </div>

                        <p className="text-gray-600 leading-relaxed">
                          Time series analysis (2015-2024) shows that global{" "}
                          {selectedFactor.toLowerCase()} has
                          <span className="font-medium">
                            {" "}
                            {Math.abs(globalChange) < 5
                              ? "remained relatively stable"
                              : globalChange > 0
                              ? `increased by ${globalChange.toFixed(1)}%`
                              : `decreased by ${Math.abs(globalChange).toFixed(
                                  1
                                )}%`}
                          </span>
                          .
                          ...
                          {maxIncrease.key && Math.abs(maxIncrease.change) > 5
                            ? ` notable improvements in ${formattedRegion(
                                maxIncrease.key
                              )}`
                            : ""}
                          {maxDecrease.key && Math.abs(maxDecrease.change) > 5
                            ? `${
                                ...
                                Math.abs(maxIncrease.change) > 5
                                  ? " and"
                                  : ""
                              } declines in ${formattedRegion(maxDecrease.key)}`
                            : ""}
                          .
                        </p>
                      </div>
              ...
              <div className="p-6">
                ...
                {factorImpactData.length > 0 &&
                  (() => {
                    ...
                    const veryHigh = factorImpactData.find(
                      (d) => d.group === "Very High"
                    );
                    const high = factorImpactData.find(
                      (d) => d.group === "High"
                    );
                    const medium = factorImpactData.find(
                      (d) => d.group === "Medium"
                    );
                    ...
                    if (veryHigh && high && medium && low) {
                      ...
                      if (increases > decreases) {
                        ...
                        trend =
                          "shows a non-linear relationship with development level";
                      }
                    }

                    if (veryHigh && low) {
                      const difference = Math.abs(
                        veryHigh.proportion - low.proportion
                      ).toFixed(1);
                      const higherIn =
                        veryHigh.proportion > low.proportion
                          ? "very high"
                      ...
                      return (
                        <div className="ml-13 pl-0">
                          ...
                          <p className="text-gray-600 leading-relaxed">
                            Analysis across development categories reveals that{" "}
                            {selectedFactor.toLowerCase()}'s contribution to
                            happiness
                            <span className="font-medium"> {trend}</span>. Data
                            shows a
                            <span className="font-medium">
                              {" "}
                              {difference}% difference
                            </span>{" "}
                            between very high and low development countries.
                            {selectedFactor === "GDP per Capita"
                              ? " Once basic economic needs are met, other factors become increasingly important for well-being."
                              : selectedFactor === "Social Support"
                              ? " Support systems remain vital regardless of economic development, though their character may change with prosperity."
                              : selectedFactor === "Life Expectancy"
                              ? " Healthcare priorities shift as nations develop, with basic health becoming a baseline expectation in advanced economies."
                              : selectedFactor === "Freedom"
                              ? " Perceptions of freedom and autonomy vary widely across development contexts and cultural frameworks."
                              : selectedFactor === "Generosity"
                              ? " Prosocial behaviors are differently motivated based on resource availability and cultural norms."
                      ...
                      return (
                        <div className="ml-13 pl-0">
                          <p className="text-gray-600">
                            Countries with balanced development across all
                            happiness factors tend to show more consistent
